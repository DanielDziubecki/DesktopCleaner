...
namespace IconRestorer.Code
{
    internal class Desktop
    {
        ...
        private readonly List<string> currentIconsOrder;
        ...
        public Desktop()
        {
            ...
            currentIconsOrder = new List<string>();
            for (AutomationElement child = walker.GetFirstChild(el);
                child != null;
                child = walker.GetNextSibling(child))
            {
                currentIconsOrder.Add(child.Current.Name);
            }
        }

        private int GetIconsNumber()
        {
            return (int)Win32.SendMessage(desktopHandle, Win32.LVM_GETITEMCOUNT, IntPtr.Zero, IntPtr.Zero);
        }
        ...
        private NamedDesktopPoint[] GetIconsPositions(IntPtr desktopProcessHandle, IntPtr sharedMemoryPointer)
        {
            ...
            for (int itemIndex = 0; itemIndex < numberOfIcons; itemIndex++)
            {
                ...
                Win32.SendMessage(desktopHandle, Win32.LVM_GETITEMPOSITION, itemIndex, sharedMemoryPointer);
                ...
                var point = points[0];
                listOfPoints.AddLast(new NamedDesktopPoint(currentIconsOrder[itemIndex], point.X, point.Y));
            }
        ...
        public void SetIconPositions(IEnumerable<NamedDesktopPoint> iconPositions)
        {
            ...
            {
                var iconIndex = currentIconsOrder.IndexOf(position.Name);
                if (iconIndex == -1)
                { continue; }

                Win32.SendMessage(desktopHandle, Win32.LVM_SETITEMPOSITION, iconIndex, Win32.MakeLParam(position.X, position.Y));
            }
        }

        public void Refresh()
        {
            Win32.PostMessage(desktopHandle, Win32.WM_KEYDOWN, Win32.VK_F5, 0);
